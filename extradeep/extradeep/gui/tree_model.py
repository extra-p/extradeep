...
from extradeep.entities import calltree
...
class TreeModel(QAbstractItemModel):
    def __init__(self, selectorWidget: SelectorWidget, call_tree: CallTree, parent=None):
    ...
    def getSelectedModel(self, callpath) -> Optional[Model]:
        ...
        if model is None or metric is None or analysistype is None:
            return None
        key = (callpath, metric, analysistype)
    ...
    def on_metric_changed(self):
        ...
        def selection_function(tree_item: TreeItem):
            key = (tree_item.call_tree_node.path, metric, analysistype)
        ...
        if self.root_item.call_tree_node is not None:
            if self.root_item.does_selection_change(selection_function):
                ...
                self.endResetModel()
            self.valuesChanged(analysistype)
    ...
    def columnCount(self, _=None):
        return 7  # This needs to be updated when a new column is added!
    ...
    def valuesChanged(self, analysistype: AnalysisType):
        self.dataChanged.emit(self.createIndex(0, 0),
                              self.createIndex(len(self.main_widget.getExperiment().callpaths[analysistype]) - 1,
