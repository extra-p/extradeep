...
from extradeep.gui.utils import formatFormula
from extradeep.gui.utils import formatNumber
...
class GraphWidget(QWidget):
    ...
    def setMax(self, axis, maxX):
        ...
        if axis == 0:
            ...
            print("[EXTRA-DEEP:] Error: Set maximum for axis other than X-axis.")
    ...
    def screenshot(self):
        ...
        if selected_analysis_type:
            name_addition = f"-{selected_analysis_type}-"
        if selected_metric:
    ...
    def exportData(self):
        ...
        for selected_callpath in selected_callpaths:
            model = model_set_models[selected_callpath.path, selected_metric, selected_analysis_type]
            if model is None:
                return
            model_function = model.hypothesis.function
            data_points = [p for (_, p) in self.calculateDataPoints(selected_analysis_type,
    ...
    def drawGraph(self, paint):
        ...
        # Draw data points
        self.drawDataPoints(paint, selected_analysis_type, selected_metric, selected_callpaths_checked, selected_callpaths_checked)

        # Draw legend
        self.drawLegend(paint)

    def drawDataPoints(self, paint, selectedAnalysisType, selectedMetric, selectedCallpaths, selected_callpaths_checked):
            ...
            for selected_callpath in selectedCallpaths:
                ...
                if self.datapoints_type == "outlier":
                    self.showOutlierPoints(
                        paint, selectedAnalysisType, selectedMetric, selected_callpath, color)

                else:
                    data_points = self.calculateDataPoints(
                        selectedAnalysisType, selectedMetric, selected_callpath)
    ...
    def calculateDataPoints(self, selectedAnalysisType, selectedMetric, selectedCallpath, ignore_limit=False):
        """ This function calculates datapoints to be marked on the graph
        """

        datapoints = self.main_widget.getCurrentModel().models[(selectedCallpath.path, selectedMetric, selectedAnalysisType)].measurements
    ...
    def calculateMaxY(self, modelList):
        ...
        # check the datapoints and their max values
        temp_max = 0
        for model in modelList:
            measurements = model.measurements
            for i in range(len(measurements)):
                max_measured = measurements[i].maximum
                if max_measured > temp_max:
                    temp_max = max_measured
        y_max = temp_max
    ...
    def showOutlierPoints(self, paint, selectedAnalysisType, selectedMetric, selectedCallpath, color):
        model_set = self.main_widget.getCurrentModel()
        datapoints = model_set.models[(selectedCallpath.path, selectedMetric, selectedAnalysisType)].measurements
        parameter_datapoint = self.main_widget.data_display.getAxisParameter(0).id
        for datapoint in datapoints:
            ...
            if x_value <= self.max_x:
                ...
                self.plotOutliers(paint, x_value, y_min_value,
                                  y_mean_value, y_median_value, y_max_value, color)

    def plotOutliers(self, paint, x_value, y_min_value, y_mean_value, y_median_value, y_max_value, color):
        ...
        data_range_color = QColor(color)
        pen = QPen(data_range_color)
