...
import extradeep
from extradeep.gui.nsight_systems_reader import NsightSystemsReader
from extradeep.modelers.model_generator import ModelGenerator
from extradeep.fileio.experiment_io import read_experiment, write_experiment
from extradeep.gui.color_widget import ColorWidget
from extradeep.gui.data_display import DataDisplayManager, GraphLimitsWidget
from extradeep.gui.log_widget import LogWidget
from extradeep.gui.modeler_widget import ModelerWidget
from extradeep.gui.plot_type_selector import PlotTypeSelector
from extradeep.gui.progress_window import ProgressWindow
from extradeep.gui.selector_widget import SelectorWidget
...
class MainWidget(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        """
        Initializes the extradeepp application widget.
        """
    ...
    def initUI(self):
        """
        Initializes the User Interface of the extradeep widget. E.g. the menus.
        """
        self.setWindowTitle(extradeep.__title__)
        # Status bar
        # self.statusBar()
        ...
        # Left side: Callpath, metric, and analysis type selection
        ...
        file_imports = [
            ('Open set of Nsight Systems .sqlite files', 'Open a set of Nsight Systems .sqlite files for single and multi parameter models and generate data points '
                                        'for a new experiment from them', self.open_nsight_systems_files)
        ]
        ...
        doc_action.triggered.connect(lambda: QDesktopServices.openUrl(QUrl(extradeep.__documentation_link__)))
    ...
    def setExperiment(self, experiment):
        ...
        self.selector_widget.fillAnalysisList()
        ...
        for i in range(len(self.selector_widget.tree_models)):
            self.selector_widget.tree_models[self.experiment.analysistypes[i]].valuesChanged(self.experiment.analysistypes[i])
    ...
    def getSelectedAnalysisType(self):
        return self.selector_widget.getSelectedAnalysisType()
    ...
    def _set_opened_file_name(self, file_name):
        if file_name:
            ...
            self.setWindowTitle(Path(file_name).name + " – " + extradeep.__title__)
    ...
    def open_nsight_systems_files(self):
        def _process_ns(dir_name):
            dialog = NsightSystemsReader(self, dir_name)
        ...
        self._file_dialog(_process_ns,
                          'Select a Directory with a Set of Nsight Systems .sqlite Files', "", file_mode=QFileDialog.Directory)
    ...
    def populateCallPathColorMap(self, call_tree_nodes):
        ...
        analysistype = self.selector_widget.getSelectedAnalysisType()
        call_tree_nodes = [c for c in set(call_tree_nodes) if (c.path, metric, analysistype) in self.selector_widget.getCurrentModel().models]
        current_index = 0
    ...
    def show_about_dialog(self):
        QMessageBox.about(self, "About " + extradeep.__title__,
                          f"""<h1>{extradeep.__title__}</h1>
<p>Version {extradeep.__version__}</p>
<p>{extradeep.__description__}</p>
<p>{extradeep.__copyright__}</p>
"""
                          )
