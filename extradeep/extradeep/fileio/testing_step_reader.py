...
from extradeep.fileio.nsight_systems_file_reader import read_nvtx_testing_steps, add_testing_steps_to_experiment

from extradeep.fileio.sqlite_helper import get_data_from_db
from extradeep.entities.system.system_info import SystemInfo


def parallel_read_testing_step(inputs):
    """
...
def read_testing_steps(folder, arguments):
    ...
    if len(paths) != 0:
        ...
        with Pool(processes=max_processes) as pool:
            parallel_result = list(tqdm(pool.imap(parallel_read_testing_step, inputs), total=len(paths), desc="Reading Nsight Systems .sqlite files"))
        ...
        with ProgressBar(desc='Data preprocessing') as pbar:
            ...
            measurement_testing_step_runtime = None
            ...
            if measurement_testing_step_runtime != None:
                ...
                extrap_experiment = add_testing_steps_to_experiment(extrap_experiment, pbar, unique_configs, measurement_testing_step_runtime, arguments, system_info)
