...
from extradeep.fileio.nsight_systems_file_reader import read_cublas_events, populate_measurement_callpath, add_cublas_events_to_experiment

from extradeep.fileio.sqlite_helper import get_data_from_db
from extradeep.entities.system.system_info import SystemInfo


def parallel_read_cublas_events(inputs):
    """
    parallel_read_cublas_events function...
    """
    ...
    if cublas_callpaths != None and cublas_visits != None and cublas_runtime != None:
        ...
        cublas_event_visits = cublas_visits
        cublas_event_runtime = cublas_runtime
        callpaths_cublas = cublas_callpaths
    ...
    data = (config, cublas_event_visits, cublas_event_runtime, callpaths_cublas)

    return data


def read_cublas(dir_name, arguments):
    ...
    if len(paths) != 0:
        ...
        with Pool(processes=max_processes) as pool:
            parallel_result = list(tqdm(pool.imap(parallel_read_cublas_events, inputs), total=len(paths), desc="Reading Nsight Systems .sqlite files"))
        ...
        for i in range(len(parallel_result)):
            ...
            if visits != None:
                cublas_event_visits[config.id] = visits
            if runtime != None:
                cublas_event_runtime[config.id] = runtime
            if callpaths != None:
                callpaths_cublas.append(callpaths)
        ...
        with ProgressBar(desc='Data preprocessing') as pbar:
            ...
            callpath_list_cublas_events = None
            measurement_visits_cublas_events = None
            measurement_runtime_cublas_events = None
            ...
            if measurement_runtime_cublas_events != None or measurement_visits_cublas_events != None or callpath_list_cublas_events != None:
                ...
                extrap_experiment = add_cublas_events_to_experiment(extrap_experiment, callpath_list_cublas_events, measurement_visits_cublas_events, measurement_runtime_cublas_events, pbar, unique_configs, arguments, system_info)
