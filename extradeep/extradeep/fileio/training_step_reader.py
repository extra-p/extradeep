...
from extradeep.fileio.nsight_systems_file_reader import read_nvtx_training_steps, add_training_steps_to_experiment

from extradeep.fileio.sqlite_helper import get_data_from_db
from extradeep.entities.system.system_info import SystemInfo


def parallel_read_training_step(inputs):
    """
...
def read_training_steps(folder, arguments):
    ...
    if len(paths) != 0:
        ...
        # create input array for parallel training step i/o
        inputs = []
        for i in range(len(paths)):
            inputs.append([i, paths[i], files[i]])

        # create process pool for parallel training step i/o
        with Pool(processes=max_processes) as pool:
            parallel_result = list(tqdm(pool.imap(parallel_read_training_step, inputs), total=len(paths), desc="Reading Nsight Systems .sqlite files"))
        ...
        with ProgressBar(desc='Data preprocessing') as pbar:
            ...
            measurement_training_step_runtime = None
            ...
            if measurement_training_step_runtime != None:
                ...
                extrap_experiment = add_training_steps_to_experiment(extrap_experiment, pbar, unique_configs, measurement_training_step_runtime, arguments, system_info)
