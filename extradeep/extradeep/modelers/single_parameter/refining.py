...
from extradeep.entities.fraction import Fraction
...
class RefiningModeler(SingularModeler, AbstractSingleParameterModeler):
    ...
    modeler_options.group('Exponents', allow_negative_exponents)
    ...
    def create_model(self, measurements):
        ...
        if self.allow_negative_exponents == True:

            allow_log = self.allow_log_terms and self.are_measurements_log_capable(measurements)
            if allow_log == True:
                log_expos = range(-self.max_log_expo - 1, self.max_log_expo + 1)
            else:
                log_expos = self.max_log_expo if allow_log else 0

            poly_expos = range(-self.max_poly_expo - 1, self.max_poly_expo + 1)
            #log_expos = range(-self.max_log_expo - 1, self.max_log_expo + 1)
        else:
            poly_expos = range(self.max_poly_expo + 1)
            log_expos = range(self.max_log_expo + 1)
