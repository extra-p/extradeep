...
class CIFAR10Benchmark():
    """
    CIFAR-10 Benchmark class. Runs simple distributed deep learning benchmark on the CIFAR-10 dataset training a specified resnet network.
    """

    @nvtx.annotate("main->init", color="green")
    def __init__(self, REPETITION, NR_PROCESSES, NETWORK, EPOCHS, BATCH_SIZE, TF_PROFILER, NR_PARAMETERS, VERBOSE, DETERMINISM, MEMORY_GROWTH, MIXED_PRECISION, TENSOR32_EXECUTION):
        """
        ...
        # analyse p the number of mpi processes
        if self.NR_PARAMETERS == 1:
            self.FOLDER_NAME = self.JOBNAME + "_p" + str(NR_PROCESSES) + "_r" + str(REPETITION)
        # analyse n the network cofiguration
        elif self.NR_PARAMETERS == 2:
            network = self.NETWORK[6:]
            self.FOLDER_NAME = self.JOBNAME + "_p" + str(NR_PROCESSES) + "_b" + str(BATCH_SIZE) + "_r" + str(REPETITION)

        #self.FOLDER_NAME = self.JOBNAME + "_p" + str(NR_PROCESSES) + "_n" + network + "_r" + str(REPETITION)
