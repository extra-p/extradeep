"""
See for details: https://github.com/gahaalt/resnets-in-tensorflow2
"""
...
@nvtx.annotate("main->define_network->regularized_padded_conv", color="pink")
def regularized_padded_conv(*args, **kwargs):
    return tf.keras.layers.Conv2D(*args, **kwargs, padding='same', kernel_regularizer=_regularizer,
                                  kernel_initializer='he_normal', use_bias=False)

@nvtx.annotate("main->define_network->bn_relu", color="pink")
def bn_relu(x):
    x = tf.keras.layers.BatchNormalization()(x)
    return tf.keras.layers.ReLU()(x)

@nvtx.annotate("main->define_network->shortcut", color="pink")
...
@nvtx.annotate("main->define_network->original_block", color="pink")
...
@nvtx.annotate("main->define_network->preactivation_block", color="pink")
...
@nvtx.annotate("main->define_network->bootleneck_block", color="pink")
...
@nvtx.annotate("main->define_network->group_of_blocks", color="pink")
...
@nvtx.annotate("main->define_network->Resnet", color="pink")
def Resnet(input_shape, n_classes, l2_reg=1e-4, group_sizes=(2, 2, 2), features=(16, 32, 64), strides=(1, 2, 2),
    ...
    outputs = tf.keras.layers.Dense(n_classes, activation="softmax", dtype="float32")(flow)
    model = tf.keras.Model(inputs=inputs, outputs=outputs)
    return model

@nvtx.annotate("main->define_network->load_weights_func", color="pink")
...
@nvtx.annotate("main->define_network->cifar_resnet20", color="pink")
...
@nvtx.annotate("main->define_network->cifar_resnet32", color="pink")
...
@nvtx.annotate("main->define_network->cifar_resnet44", color="pink")
...
@nvtx.annotate("main->define_network->cifar_resnet56", color="pink")
...
@nvtx.annotate("main->define_network->cifar_resnet110", color="pink")
def cifar_resnet110(num_classes, block_type='preactivated', shortcut_type='B', l2_reg=1e-4, load_weights=False):
    model = Resnet(input_shape=(32, 32, 3), n_classes=num_classes, l2_reg=l2_reg, group_sizes=(18, 18, 18), features=(16, 32, 64),
                   strides=(1, 2, 2), first_conv={"filters": 16, "kernel_size": 3, "strides": 1}, shortcut_type=shortcut_type,
                   block_type=block_type, preact_shortcuts=False)
    if load_weights: model = load_weights_func(model, 'cifar_resnet110')
    return model

@nvtx.annotate("main->define_network->cifar_resnet164", color="pink")
def cifar_resnet164(num_classes, shortcut_type='B', load_weights=False, l2_reg=1e-4):
    model = Resnet(input_shape=(32, 32, 3), n_classes=num_classes, l2_reg=l2_reg, group_sizes=(18, 18, 18), features=(64, 128, 256),
                   strides=(1, 2, 2), first_conv={"filters": 16, "kernel_size": 3, "strides": 1}, shortcut_type=shortcut_type,
                   block_type='bootleneck', preact_shortcuts=True)
    if load_weights: model = load_weights_func(model, 'cifar_resnet164')
    return model

@nvtx.annotate("main->define_network->cifar_resnet1001", color="pink")
def cifar_resnet1001(num_classes, shortcut_type='B', load_weights=False, l2_reg=1e-4):
    model = Resnet(input_shape=(32, 32, 3), n_classes=num_classes, l2_reg=l2_reg, group_sizes=(111, 111, 111), features=(64, 128, 256),
                   strides=(1, 2, 2), first_conv={"filters": 16, "kernel_size": 3, "strides": 1}, shortcut_type=shortcut_type,
                   block_type='bootleneck', preact_shortcuts=True)
    if load_weights: model = load_weights_func(model, 'cifar_resnet1001')
    return model

@nvtx.annotate("main->define_network->cifar_wide_resnet", color="pink")
...
@nvtx.annotate("main->define_network->cifar_WRN_16_4", color="pink")
...
@nvtx.annotate("main->define_network->cifar_WRN_40_4", color="pink")
...
@nvtx.annotate("main->define_network->cifar_WRN_16_8", color="pink")
...
@nvtx.annotate("main->define_network->cifar_WRN_28_10", color="pink")
...
@nvtx.annotate("main->define_network->cifar_resnext", color="pink")
