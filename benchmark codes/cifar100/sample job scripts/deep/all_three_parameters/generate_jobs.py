...
for i in range(len(tasks)):
    ...
    for j in range(len(nets)):
        ...
        for k in range(len(batches)):
            ...
            file = open("../../../cifar100_p"+str(tasks[i])+"_n"+str(nets[j])+"_b"+str(batches[k])+"_job_all3.sh","w")
            ...
            text += "#SBATCH -n "+str(tasks[i])+"\n"
            text += "#SBATCH -o /<replace/cifar100-benchmark/all3/output_p"+str(tasks[i])+"_n"+str(nets[j])+"_b"+str(batches[k])+".out\n"
            text += "#SBATCH -e /<replace/cifar100-benchmark/all3/error_p"+str(tasks[i])+"_n"+str(nets[j])+"_b"+str(batches[k])+".er\n"
            ...
            text += "\n"
            text += "PSP_OPENIB=1 PSP_UCP=0 srun /usr/local/software/skylake/Stages/2020/software/CUDA/11.3/bin/nsys profile -t nvtx,cuda,mpi,cublas,cudnn --mpi-impl mpich -b none --cpuctxsw none -f true -x true -o /p/project/deepsea/ritter2/cifar100-benchmark/all3/cifar100.p"+str(tasks[i])+".n"+str(nets[j])+".b"+str(batches[k])+".r1.mpi%q{SLURM_PROCID} python -u main.py -p "+str(tasks[i])+" -r 1 -n resnet"+str(nets[j])+" -b "+str(batches[k])+" -nrparameters 3 -e 2 -mode all\n"
            file.write(text)
