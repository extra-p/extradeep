...
for i in range(len(tasks)):
    ...
    for j in range(len(nets)):
        ...
        file = open("../../../cifar100_p"+str(tasks[i])+"_n"+str(nets[j])+"_job_all2.sh","w")
        ...
        text += "#SBATCH -n "+str(tasks[i])+"\n"
        text += "#SBATCH -o /<replace/cifar100-benchmark/all2/output_p"+str(tasks[i])+"_n"+str(nets[j])+".out\n"
        text += "#SBATCH -e /<replace/cifar100-benchmark/all2/error_p"+str(tasks[i])+"_n"+str(nets[j])+".er\n"
