...
class CIFAR100Benchmark():
    """
    CIFAR-100 Benchmark class. Runs simple distributed deep learning benchmark on the CIFAR-100 dataset training a specified resnet network.
    """
    ...
    def __init__(self, REPETITION, NR_PROCESSES, NETWORK, EPOCHS, BATCH_SIZE, TF_PROFILER, NR_PARAMETERS, VERBOSE, DETERMINISM, MEMORY_GROWTH, MIXED_PRECISION, TENSOR32_EXECUTION, PROBLEM_SIZE, MODE):
        ...
        # for three parameter analysis
            ...
            network = self.NETWORK[6:]
            self.FOLDER_NAME = self.JOBNAME + "_p" + str(NR_PROCESSES) + "_n" + network + "_b"+str(BATCH_SIZE)+"_r" + str(REPETITION) + "_" + str(self.MODE)
    ...
    def load_data(self):
        ...
        # load the number of classes from the dataset
        self.classes = data_loader.get_classes()
    ...
    @nvtx.annotate("main->scale_problemsize", color="purple")
    def scale_problemsize(self):
        """
        This functions adjusts the problem size of the benchmark.
        It adjusts the number of classes of images to detect to modulate the problem size.
        """

        # init the problem scaler
        scaler = ProblemScaler(self.classes, self.trainX, self.trainY, self.testX, self.testY)

        # scale the problem size
        self.trainX, self.trainY, self.testX, self.testY = scaler.scale_problem(self.PROBLEM_SIZE)
    ...
    def preprocessing(self):
        ...
        preprocessor = Preprocessor(self.trainX, self.testX, self.trainY, self.testY, self.BATCH_SIZE, self.PROBLEM_SIZE, self.EPOCHS, hvd.size(), hvd.rank())
    ...
    def define_network(self):
        ...
        if self.NETWORK == "resnet18":
            ...
            self.model = resnet.define_resnet18(input_shape=(32,32,3), num_outputs=self.PROBLEM_SIZE)
            ...
            resnet = ResNet(weigt_decay=self.WEIGHT_DECAY)
            self.model = resnet.define_resnet34(input_shape=(32,32,3), num_outputs=self.PROBLEM_SIZE)
            self.model = resnet.compile_model(self.model, learning_rate=self.LEARNING_RATE, momentum=self.MOMENTUM)
        elif self.NETWORK == "resnet50":
            resnet = ResNet(weigt_decay=self.WEIGHT_DECAY)
            self.model = resnet.define_resnet50(input_shape=(32,32,3), num_outputs=self.PROBLEM_SIZE)
            self.model = resnet.compile_model(self.model, learning_rate=self.LEARNING_RATE, momentum=self.MOMENTUM)
        elif self.NETWORK == "resnet101":
            resnet = ResNet(weigt_decay=self.WEIGHT_DECAY)
            self.model = resnet.define_resnet101(input_shape=(32,32,3), num_outputs=self.PROBLEM_SIZE)
            self.model = resnet.compile_model(self.model, learning_rate=self.LEARNING_RATE, momentum=self.MOMENTUM)
        elif self.NETWORK == "resnet152":
            resnet = ResNet(weigt_decay=self.WEIGHT_DECAY)
            self.model = resnet.define_resnet152(input_shape=(32,32,3), num_outputs=self.PROBLEM_SIZE)
            self.model = resnet.compile_model(self.model, learning_rate=self.LEARNING_RATE, momentum=self.MOMENTUM)
        elif self.NETWORK == "resnet110":
            self.model = cifar_resnet110(self.PROBLEM_SIZE, l2_reg=self.WEIGHT_DECAY)
            ...
            self.model = cifar_resnet164(self.PROBLEM_SIZE, l2_reg=self.WEIGHT_DECAY)
            opt = SGD(learning_rate=self.LEARNING_RATE, momentum=self.MOMENTUM)
            # Horovod: add Horovod DistributedOptimizer on top of the standard optimizer
            opt = hvd.DistributedOptimizer(opt)
            # INFO: the next line should help with numerical stability
            ...
            self.model = cifar_resnet1001(self.PROBLEM_SIZE, l2_reg=self.WEIGHT_DECAY)
            opt = SGD(learning_rate=self.LEARNING_RATE, momentum=self.MOMENTUM)
            # Horovod: add Horovod DistributedOptimizer on top of the standard optimizer
            opt = hvd.DistributedOptimizer(opt)
            # INFO: the next line should help with numerical stability
    ...
    def train(self):
        ...
        if self.MODE == "nvtx":
            ...
            #print("train data set size:",self.trainX.shape[0])
            #print("test data set size:",validation_size)
            ...
            #print("test steps:",validation_steps)
