...
for i in range(len(nodes)):
    ...
    file = open("../../imagenet_p"+str(tasks[i])+"_job_all.sh","w")
    ...
    text += "#SBATCH -o mpi_ranks_all/output_p"+str(tasks[i])+"_all.out\n"
    text += "#SBATCH -e mpi_ranks_all/error_p"+str(tasks[i])+"_all.er\n"
    text += "#SBATCH --time=01:00:00\n"
    text += "#SBATCH -J img"+str(tasks[i])+"all\n"
    ...
    text += "srun nsys profile -t nvtx,cuda,mpi --mpi-impl openmpi -b none --cpuctxsw none -f true -x true -o mpi_ranks_all/imagenet.p"+str(tasks[i])+".r1.mpi%q{SLURM_PROCID} python -u main.py -p "+str(tasks[i])+" -r 1 -e 1 --mode all\n"
