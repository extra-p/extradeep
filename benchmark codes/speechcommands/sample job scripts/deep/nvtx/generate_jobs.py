...
for i in range(len(tasks)):
    ...
    file = open("../../../speech_p"+str(tasks[i])+"_job.sh","w")
    ...
    text += "\n"
    text += "PSP_OPENIB=1 PSP_UCP=0 srun /usr/local/software/skylake/Stages/2020/software/CUDA/11.3/bin/nsys profile -t nvtx --mpi-impl mpich -b none --cpuctxsw none -f true -x true -o /p/project/deepsea/ritter2/speechcommands-benchmark/speech.p"+str(tasks[i])+".r1.mpi%q{SLURM_PROCID} python -u main.py -p "+str(tasks[i])+" -r 1\n"
    file.write(text)
