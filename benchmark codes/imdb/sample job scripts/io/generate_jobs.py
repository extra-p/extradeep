...
for i in range(len(nodes)):
    ...
    file = open("../../imdb_p"+str(tasks[i])+"_job_io.sh","w")
    ...
    text += "export DARSHAN_LOGFILE=darshanlogs/imdb_p"+str(tasks[i])+"_r1.log\n"
    text += "export DXT_ENABLE_IO_TRACE=1\n"
    text += "\n"
    text += "# Run the program\n"
    text += "srun python -u main.py -p "+str(tasks[i])+" -r 1\n"
